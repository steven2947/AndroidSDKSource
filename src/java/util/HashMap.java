...
public class HashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
     ...
     *
     * @param initialCapacity the initial capacity 初始容量，是哈希表创建中桶的数量。
     * @param loadFactor      the load factor 加载因子(默认0.75)，是哈希表在其容量自动增加之前可以达到多满的一种尺度。
    ...
    public V put(K key, V value) {
        ...
        //允许存放null的key和null为value
        //当key为null时调用putForNullKey方法.放入到table[0]的位置
        if (key == null)
